(function () {
	...
	function changePlugin()
	{
		...
		{
			if (plug.url.indexOf('sisi.am/nyam.js') >= 0)
			{
				updateplugins = true;
				plug.url = (plug.url + '').replace('sisi.am/nyam.js', 'sisi.am/pussy.js');
			}
		})
    ...
    function menu$2(target, card_data) {
      ...
      Lampa.Select.show({
        title: 'ÐœÐµÐ½ÑŽ',
        items: [{
          title: !card_data.bookmark.uid ? 'Ð’ Ð·Ð°ÐºÐ»Ð°Ð´ÐºÐ¸' : 'Ð£Ð´Ð°Ð»Ð¸Ñ‚ÑŒ Ð¸Ð· Ð·Ð°ÐºÐ»Ð°Ð´Ð¾Ðº'
        }],
        onSelect: function onSelect() {
          Api.bookmark(card_data, !card_data.bookmark.uid, function (status) {
            Lampa.Noty.show('Ð£ÑÐ¿ÐµÑˆÐ½Ð¾');
          });
        ...
        Api.view(object, function (data) {
          ...
          if (menu) {
            menu.forEach(function (m) {
              ...
              m.title = spl[0].trim();
              if (spl[1]) m.subtitle = Lampa.Utils.capitalizeFirstLetter(spl[1].trim().replace(/all/i, 'Ð›ÑŽÐ±Ð¾Ð¹'));

              if (m.submenu) {
                m.submenu.forEach(function (s) {
                  s.title = Lampa.Utils.capitalizeFirstLetter(s.title.trim().replace(/all/i, 'Ð›ÑŽÐ±Ð¾Ð¹'));
                });
              }
            });
          }
          ...
          if (!data.results.length && object.url.indexOf('/bookmarks')) {
            Lampa.Noty.show('Ð£Ð´ÐµÑ€Ð¶Ð¸Ð²Ð°Ð¹Ñ‚Ðµ ÐžÐš Ð½Ð° Ð²Ð¸Ð´ÐµÐ¾ Ð´Ð»Ñ Ð´Ð¾Ð±Ð°Ð²Ð»ÐµÐ½Ð¸Ñ Ð² Ð·Ð°ÐºÐ»Ð°Ð´ÐºÐ¸.', {
              time: 10000
            });
          }
    ...
    function startPlugin() {
      ...
      function addSettings() {
        ...
        Lampa.SettingsApi.addParam({
          ...
          field: {
            name: 'ÐŸÑ€ÐµÐ´Ð¿Ñ€Ð¾ÑÐ¼Ð¾Ñ‚Ñ€',
            description: 'ÐŸÐ¾ÐºÐ°Ð·Ñ‹Ð²Ð°Ñ‚ÑŒ Ð¿Ñ€ÐµÐ´Ð¿Ñ€Ð¾ÑÐ¼Ð¾Ñ‚Ñ€ Ð¿Ñ€Ð¸ Ð½Ð°Ð²ÐµÐ´ÐµÐ½Ð¸Ðµ Ð½Ð° ÐºÐ°Ñ€Ñ‚Ð¾Ñ‡ÐºÑƒ'
          },
